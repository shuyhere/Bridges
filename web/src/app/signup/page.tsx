...
import { signup, saveSession, ApiError } from "@/lib/api";
...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const resp = await signup(email, password, displayName || undefined);
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-[380px]">
        <div className="text-center mb-8">
          ...
          <h1 className="text-[22px] font-semibold tracking-[-0.02em]">Create your account</h1>
          <p className="text-[14px] text-muted mt-1.5">Get started with Bridges in seconds</p>
        </div>
        ...
        <div className="soft-card p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Input id="name" label="Display Name" placeholder="Your name" value={displayName} onChange={(e) => setDisplayName(e.target.value)} />
        ...
        <p className="text-center text-[13px] text-muted mt-5">
          ...
          <Link href="/login" className="text-foreground underline underline-offset-2">Log in</Link>
        </p>
      </div>
    </div>
  );
